...
- Read the FIRST error first: later errors are usually cascade — one bad setting or missing package can fail every app that imports it. Ask what single cause explains the most symptoms.
...
- Never apply a fix whose causal chain you can't state (change → mechanism → error resolved); a fix that works without an explanation will regress — especially dangerous with `--fake` migrations.
- Distinguish observed (the error text), inferred (your reading of it), and assumed (active venv, settings module, DB state, applied migrations) — verify any assumption the fix depends on.
