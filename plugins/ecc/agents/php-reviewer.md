...
- Build the failure chain before reporting: concrete request/input/state → code path → wrong outcome (injection, mass assignment, N+1 storm, silent data loss). No chain, no finding.
- Try to refute each finding before reporting it — what middleware, FormRequest rule, Eloquent cast/guard, policy, or framework default would make it a non-issue? Report only what survives.
...
- Read enough surrounding context (routes, models, middleware) to know what the code is FOR before judging how it's written.
