...
- Read the FIRST error first: later errors are usually cascade — one stale `.g.dart` file or bad import can fail an entire analysis run. Ask what single cause explains the most symptoms.
...
- Distinguish observed (the error text), inferred (your reading of it), and assumed (Flutter/Dart SDK versions, pub constraints, platform toolchain) — verify any assumption the fix depends on.
