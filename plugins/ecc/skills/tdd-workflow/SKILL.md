...
1. **Detect the package manager** by inspecting the project:
   ...
   Resolve the package manager (npm / pnpm / yarn / bun) by checking, in order: the `package.json` `packageManager` field, then the lockfile — `package-lock.json` → npm, `pnpm-lock.yaml` → pnpm, `yarn.lock` → yarn, `bun.lockb` → bun.
