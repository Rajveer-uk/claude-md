...
description: Kubernetes workload patterns, resource management, RBAC, probes, autoscaling, ConfigMap/Secret handling, and kubectl debugging for production-grade deployments. Use when writing or reviewing Kubernetes manifests.
metadata:
  origin: ECC
---
...
1. **Deployment template** — production `Deployment` with security context, rolling update strategy, all three probe types, resource limits, ConfigMap/Secret env injection.
2. **Probes** — startup vs liveness vs readiness decision table, with `failureThreshold × periodSeconds` math.
3. **Services & Ingress** — ClusterIP, LoadBalancer, TLS Ingress with cert-manager annotations.
4. **ConfigMaps & Secrets** — `envFrom`, file-mount, external secrets guidance.
...
7. **HPA & PDB** — autoscaling and node-drain safety.
8. **Jobs & CronJobs** — one-off and scheduled workloads with correct `restartPolicy`.
9. **kubectl cheatsheet** — logs, exec, rollback, port-forward, dry-run, common error diagnosis.
10. **Anti-patterns & checklist** — what NOT to do; security/reliability/observability checklist.

## Examples

Quick references (complete, runnable examples below):
...
Prevents too many pods going down during node drains or rolling updates:
